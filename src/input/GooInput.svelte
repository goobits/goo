...
<script lang="ts" generics="T = string">
	...
	$effect(() => {
		...
		untrack(() =>
			Object.defineProperties(element, {
				...
				setValue: {
					configurable: true,
					value: (value: T, options: { silent?: boolean } = {}) =>
	...
	function handleFocus(event: Event): void {
		...
		inputElement?.dispatchEvent(
			new CustomEvent('focus', { detail: { value: currentValue, target: inputElement } })
		)
	}
	...
	function handleBlur(event: Event): void {
		...
		onblur?.(currentValue)
		inputElement?.dispatchEvent(
			new CustomEvent('blur', { detail: { value: currentValue, target: inputElement } })
		)
	}
