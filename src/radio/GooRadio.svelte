<script lang="ts">
	import { handleKeyboardActivation } from '@goobits/keyboard'
	...
	function check(options: { silent?: boolean } = {}): void {
		...
		if (name && !radioElement?.closest('.goo-radio-group')) {
			for (const sibling of document.querySelectorAll<HTMLButtonElement>(
				`.goo-radio[name="${cssEscape(name)}"]`
			)) {
				if (sibling !== radioElement && 'uncheck' in sibling) {
					;(
						sibling as HTMLButtonElement & { uncheck(options?: { silent?: boolean }): void }
					).uncheck({ silent: true })
				}
			}
		}
