<script lang="ts" generics="T">
	...
	} from '../virtualGrid/virtualWindow.ts'
	...
	function updateWindow(): void {
		...
		const viewportHeight =
			effectiveScrollRoot.clientHeight || rowHeight * Math.min(sortedRows.length, 12)
		setWindowState(
			calculateVirtualGridWindow({
				columns: 1,
				itemsTop: 0,
	...
	$effect(() => {
		...
		const resizeObserver =
			typeof ResizeObserver === 'undefined' ? undefined : new ResizeObserver(updateWindow)
...
>
	<div class="goo-data-grid__header" role="rowgroup">
		<div
			class="goo-data-grid__row goo-data-grid__row--header"
			role="row"
			aria-rowindex={1}
