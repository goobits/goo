...
<script lang="ts">
	...
	import { containKeyboardEvent, handleKeyboardActivation } from '../support/keyboard/_keyboardActivation.ts'
	...
	let incomingChecked = $derived(Boolean(checked))
	let currentChecked = $state(false)
	let dragState = $state<{ startX: number; width: number; offset: number; moved: boolean } | null>(
		null
	)
	...
	function emitChange(oldValue: boolean): void {
		...
		checkboxElement.dispatchEvent(
			new CustomEvent('change', {
				bubbles: true,
				detail: { value: currentChecked, oldValue, target: checkboxElement }
			})
		)
	}
...
>
	...
	{#if name}
		<input
			class="goo-checkbox__input"
			type="hidden"
			{name}
			value={formValue}
			disabled={disabled || !currentChecked}
		/>
	{/if}
	{#if children}
